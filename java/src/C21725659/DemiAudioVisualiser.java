...
public class DemiAudioVisualiser extends Visual {
    ...
    public void draw() {
        ...
        strobeTimer += getAmplitude();
        if (strobeTimer > strobeInterval) {
            strobeInterval = random(10, 30);
            strobeTimer = 0;
            background(0, 0, 0, map(getAmplitude(), 0, 1, 0, 255));
        } else {
            background(0);
        }
        noFill();
    ...
    public void displayCurrentSubtitle() {
        ...
        if (currentSubtitleIndex >= 0 && currentSubtitleIndex < subtitles.size()) {
            ...
            pushMatrix(); // Reset the matrix
            resetMatrix();
            text(currentSubtitle, 0, 0); // Adjust the
            popMatrix();
